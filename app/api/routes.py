...
async def _save_uploaded_file(upload: UploadFile | None) -> str | None:
    ...
    """
    if upload is None:
        logger.debug("[_save_uploaded_file] No file provided")
        return None
    ...
    logger.info("[_save_uploaded_file] ✅ Saved %s → %s (%d bytes)", filename, target_path, len(contents))
...
async def analyze(
    ...
    if instruction_manual is not None:
        modalities.append(Modality.TEXT)
        logger.info("[analyze] ✅ instruction_manual detected → added TEXT modality")
    if pump_image is not None:
        modalities.append(Modality.VISION)
        logger.info("[analyze] ✅ pump_image detected → added VISION modality")
    ...
    if hist_path:
        ...
        logger.info("[analyze] ✅ historical_logs detected (path=%s) → added HISTORICAL modality", hist_path)
    else:
        logger.info("[analyze] ⚠️  no historical_logs uploaded or saved")
    ...
    if hist_path:
        ...
        logger.info("[analyze] ✅ historical_logs_path added to state_dict: %s", hist_path)
    if wdl_path:
        state_dict["work_done_logs_path"] = wdl_path
        logger.info("[analyze] ✅ work_done_logs_path added to state_dict: %s", wdl_path)
    if ss_path:
        state_dict["service_schedules_path"] = ss_path
        logger.info("[analyze] ✅ service_schedules_path added to state_dict: %s", ss_path)
    if mr_path:
        state_dict["maintenance_requests_path"] = mr_path
        logger.info("[analyze] ✅ maintenance_requests_path added to state_dict: %s", mr_path)

    logger.info(
        "[analyze] final modalities=%s, state_dict keys=%s",
    ...
    if result_state.fused_score is not None:
        logger.info(
        ...
        return {
            ...
            "vision_summary": getattr(result_state, "vision_summary", None),
            "vision_result": result_state.vision_result.model_dump() if getattr(result_state, "vision_result", None) else None,
            "manual_evidence_summary": getattr(result_state, "manual_evidence_summary", None),
            "service_age_details": {
                "service_age_risk_score": getattr(result_state, "service_age_risk_score", 0.0),
                "transactional_risk_score": getattr(result_state, "transactional_risk_score", 0.0),
                "overdue_tasks": getattr(result_state, "overdue_tasks", []),
                "open_requests": getattr(result_state, "open_requests", []),
            },
            "anomaly_query": getattr(result_state, "anomaly_query", ""),
        }
