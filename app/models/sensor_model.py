...
def _load_model():
    ...
    try:
        ...
        logger.info("✅ SUCCESS: Model loaded from %s", model_path)
        _model = booster
    except Exception as e:
        logger.error("❌ CRITICAL ERROR: Could not load LightGBM file %s. Reason: %s", model_path, e)
        _model = None
        return None
...
def _reading_to_features(reading: SensorReading) -> np.ndarray:
    """Convert a SensorReading into a 1 × n array matching model's _feature_cols order."""
...
async def predict_sensor(reading: SensorReading) -> SensorPrediction:
    """Run LightGBM inference on a single sensor reading."""
    model = _load_model()
    features = _reading_to_features(reading)
    if model is None:
        logger.warning("LightGBM model not loaded; returning failure_probability=0.0")
...
def train_model(csv_path: str, target_col: str = "Maintenance_Flag") -> None:
    """Train a LightGBM binary classifier from CSV."""
    import lightgbm as lgb
